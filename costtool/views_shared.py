       ...
       #f.close()
          try:
             ...
             for u in users:
                ...
                ret['startDate'] = u.startDate
                ret['endDate'] = u.endDate
                ret['lastLogin'] = u.lastLogin
...
def export_users(request):
    ...
    try:
       ...
       for row in results:
          ...
          organisation = row[12]
          position = row[13]
          publicOrPrivate = row[14]
          startDate = row[15]
          endDate = row[16]
          lastLogin = row[17]
          timesLoggedin = row[18]
          projcount = row[19]
          for col_num in xrange(len(row)):
              if col_num == 15 or col_num == 16 or col_num == 17:
                ws.write(row_num, col_num, row[col_num],date_style4)
              else:
                ws.write(row_num, col_num, row[col_num],font_style4)
          ...
          if adminform.is_valid():
             ...
             if a.oldemail  == '' and a.email == '' and a.emailagain == '' and a.oldpassword  == '' and a.password == '' and a.passwordagain == '':
                ...
                if a.oldemail != '' or a.email != '' or a.emailagain != '':
                   try:
                      l = m.Login.objects.get(user=loggedinuser, email=a.oldemail)
                   ...
                   try:
                      p = m.Login.objects.get(user=loggedinuser, password=a.oldpassword) 
...
def program_list(request,project_id):
    ...
    return render_to_response(
            ...
            {'project':project,'program':program,'loggedinuser':loggedinuser})
...
def share_project(request, project_id):
    ...
    MFormSet = modelformset_factory(m.Projects, form=ShareProjForm, extra=10)
    if request.method == 'POST':
       shareform = MFormSet(request.POST, request.FILES)
       if shareform.is_valid():
          proj1 = shareform.save(commit=False)
          shared_user = ""
          for p in proj1:
              #f.write(proj.shared_user)
              #f.write('\n')
              if (p.shared_user == loggedinuser):
                  return render_to_response('project/share_project.html',{'shareform':shareform,'shareduser': p.shared_user, 'err1':'The user name you have entered is ', 'err2':'. You cannot enter your own name. Please enter a valid user name.'}, context) 
              try:
                 login = m.Login.objects.filter(user=p.shared_user).latest('startDate')
              except:
                  if (p.shared_user != proj.shared_user):  
                     return render_to_response('project/share_project.html',{'shareform':shareform,'shareduser': p.shared_user,'err1':'The user name you have entered is ', 'err2':'. It does not exist. Please enter a valid user name.'}, context)    
              shared_user = p.shared_user + "," + shared_user
          shared_user = shared_user.rstrip(',')
          #if proj.shared_user is None:
          proj.shared_user = shared_user 
          #else:
             #proj.shared_user = proj.shared_user + "," + shared_user 
          #f.write(proj.shared_user)
          #f.write(' before save \n')
          proj.shared = 'Y'   
          proj.updated_at = datetime.datetime.now()
          proj.shared_date = datetime.datetime.now()
          proj.save(update_fields=['shared','shared_user', 'shared_date','updated_at'])
          return HttpResponseRedirect('/project/project_list.html')
       else:
          print shareform.errors
          return render_to_response( 'project/share_project.html',{'shareform':shareform, 'err':shareform.errors},context)
    else:
       qset = m.Projects.objects.filter(pk=project_id)
       shareform = MFormSet(queryset=qset)
       #shareform = MFormSet(queryset=qset, initial = [{'shared_user': obj.shared_user.strip(',')} for obj in qset])
       #transferform = MFormSet(queryset=m.Transfers.objects.none(),initial=[{'grantYear': "%d" % (i+1)} for i in range(10)])  
       mylist = []
       for form in shareform:
           ...
           if instance and instance.shared_user:
              ...
              mylist = instance.shared_user.split(",")
              for item in mylist:
                  f.write("%s\n" % item)
                  #form.fields['shared_user'].widget.attrs['value'] = "item"
                  #form.data['shared_user'] = "item"
                  #form.fields['shared_user'].initial = "this better work"
              #for s in instance.shared_user.strip(','):
                 #form.fields['shared_user'].widget.attrs['value'] = s 
                 #mylist.append((s[0]))
                 #f.write(s[0])
                 #f.write('\n')
              #instance.shared_user = instance.shared_user.strip(',')
    f.close()
    return render_to_response(
            'project/share_project.html',{'shareform':shareform, 'mylist':mylist},
             context)
...
def unshare_project(request, project_id):
    ...
    if 'user' in request.session:
       ...
       loggedinuser = 'ccc'
    context = RequestContext(request)
    MFormSet = modelformset_factory(m.Projects, form=UnShareProjForm, extra=10)
    if request.method == 'POST':
       unshareform = MFormSet(request.POST, request.FILES)
       if unshareform.is_valid():
          proj1 = unshareform.save(commit=False)
          shared_user = ""                                                                                                                                                                                       
          for p in proj1:
              #f.write(proj.shared_user)
              #f.write('\n')
              if (p.shared_user == loggedinuser):
                  return render_to_response('project/unshare_project.html',{'unshareform':unshareform,'shareduser': p.shared_user, 'err1':'The user name you have entered is ', 'err2':'. You cannot enter your own name. Please enter a valid user name.'}, context)   
              try:
                 login = m.Login.objects.filter(user=p.shared_user).latest('startDate')
              except:
                  if (p.shared_user != proj.shared_user):  
                     return render_to_response('project/unshare_project.html',{'unshareform':unshareform,'shareduser': p.shared_user,'err1':'The user name you have entered is ', 'err2':'. It does not exist. Please enter a valid user name.'}, context)
              shared_user = p.shared_user + "," + shared_user
          shared_user = shared_user.rstrip(',')
          #if proj.shared_user is None:
          proj.shared_user = shared_user 
          ...
          print unshareform.errors
          return render_to_response( 'project/unshare_project.html',{'unshareform':unshareform, 'err':unshareform.errors},context)
    else:
       qset = m.Projects.objects.filter(pk=project_id)                                                                                                                                       
       unshareform = MFormSet(queryset=qset)
       for form in unshareform:
           instance = getattr(form, 'instance', None)
           if instance and instance.shared_user:
              form.fields['shared_user'].widget.attrs['readonly'] = True 
    #f.close()
    return render_to_response(
            'project/unshare_project.html',{'unshareform':unshareform},
             context)
...
def dupl_project(request, project_id):
    ...
    for prog in m.Programs.objects.filter(projectId = project_id):
       ...
       prog2.progname = prog.progname + '2'
...
def project_list(request):
    ...
    # start coding Apr 25   
    f = open( '/home/amritha/costtool/documents/filename.txt', 'w+' )
    if loggedinuser in ('amritha_yahoo', 'ammtest', 'amritha'):
    #'fh4','Atsuko Muroga'    
       #t = "ammtest,amritha"
       #for x in t.split(','):
          #f.write(x)
       ...
       # 1. filter projects initially by checking user is loggedinuser 
       # 2. get list of Projects that are shared, check if loggedinuser is one of the shared users, and if so, add that project to the Project list
       for p in m.Projects.objects.filter(shared = 'Y'):
           for x in p.shared_user.split(','):
              #f.write(x)
              #f.write('\n')
              if x == loggedinuser:
                 allprojects = allprojects | m.Projects.objects.filter(id = p.id)
    else:
       allprojects = m.Projects.objects.filter(user = loggedinuser) 
    #for a in allprojects:
       #if a.id == 5070: 
           
          #f.write(a.shared_user)
          #f.write(a.shared)
          #f.write('\n')
    f.close()
    # end coding Apr 25
