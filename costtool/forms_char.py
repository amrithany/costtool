...
class SettingsForm(forms.ModelForm):
    ...
    discountRateEstimates = forms.CharField(required=False,initial=3,label="Discount Rate for programs in which costs are incurred over multiple years (input number from 0 to 100):")
    ...
    def clean_discountRateEstimates(self):
        discountRateEstimates = self.cleaned_data['discountRateEstimates']
        if float(discountRateEstimates) < 0 or float(discountRateEstimates) > 100:
           raise forms.ValidationError("Discount Rate Estimates must be greater than 0 and less than 100.")
        else:
            pass
            return discountRateEstimates

    def clean_hrsCalendarYr(self):
        hrsCalendarYr = self.cleaned_data['hrsCalendarYr']
        if int(hrsCalendarYr) < 0:
           raise forms.ValidationError("Number of hours in the calendar year must be greater than 0.")
        else:
           pass
           return hrsCalendarYr

    def clean_hrsAcademicYr(self):
        hrsAcademicYr = self.cleaned_data['hrsAcademicYr']
        if int(hrsAcademicYr) < 0:
           raise forms.ValidationError("Number of years in the K-12 academic year must be greater than 0.")
        else:
           pass
           return hrsAcademicYr

    def clean_hrsHigherEdn(self):
        hrsHigherEdn = self.cleaned_data['hrsHigherEdn']
        if int(hrsHigherEdn) < 0:
           raise forms.ValidationError("Number of years in the higher education year must be greater than 0.")
        else:
           pass
           return hrsHigherEdn
...
class ProgramDescForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
             ...
             'lengthofprogram',   
            HTML("""<font color="blue">Once you save this page you cannot change the number of years over which this program operates unless you start it over as a new program.</font><br><br>"""),
            Div(
    ...
    #def clean_numberofparticipants(self):
        #numberofparticipants = self.cleaned_data['numberofparticipants']
        #if numberofparticipants.isnumeric():
           #if float(numberofparticipants) < 0:
              #raise forms.ValidationError("Average Number of participants must be greater than 0.")
        #else:
           #pass
...
class NonPerIndicesForm(forms.ModelForm):
   ...
   interestRate = forms.DecimalField(required=False, initial=0.0, min_value = 0.0, max_digits=7,decimal_places=3, label="Interest rate:")
   yearPrice = forms.CharField(required=False, widget = forms.TextInput(attrs={'readonly':'readonly'}),label="Year of the listed price:")
   statePrice  = forms.CharField(required=False, widget = forms.TextInput(attrs={'readonly':'readonly'}), label="To which geographical location does this price correspond to?")
   areaPrice = forms.CharField(required=False, widget = forms.TextInput(attrs={'readonly':'readonly'}), label="")
   sourcePriceData = forms.CharField(required=False,widget = forms.TextInput(attrs={'readonly':'readonly'}),label="Source:")
...
class PriceSummary(forms.ModelForm):
   ...
   quantityUsed  = forms.CharField(label="Quantity of ingredient needed:")
   ...
   percentageofUsage  = forms.CharField(label="Percentage of Usage", initial=100)
   lifetimeAsset = forms.IntegerField(required=False,initial=1, min_value = 1,label="Lifetime of the asset:" )                        
   ...
   def clean_percentageofUsage(self):
       percentageofUsage = self.cleaned_data['percentageofUsage']
       if float(percentageofUsage) < 0.00 or float(percentageofUsage) > 100.00:
          raise forms.ValidationError("Percentage of time used must be greater than 0 and less than 100.")
       else:
          pass
...
class MultipleSummary(forms.ModelForm):
   ...
   percentageofUsage  = forms.CharField(label="Percentage of Usage")
   lifetimeAsset = forms.IntegerField(required=False,initial=1, min_value = 1,label="Lifetime of the asset:" )                        
   interestRate = forms.DecimalField(required=False, initial=3.5, min_value = 0.0, max_digits=7,decimal_places=3, label="Interest rate:")
   ...
   def clean_percentageofUsage(self):
       percentageofUsage = self.cleaned_data['percentageofUsage']
       if float(percentageofUsage) < 0 or float(percentageofUsage) > 100:
          raise forms.ValidationError("Percentage of time used must be greater than 0 and less than 100.")
       else:
          pass
          return percentageofUsage
