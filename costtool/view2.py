...
def test(request):
    ...
    try:
       ...
       for p in program:
          ...
          try:
             ...
             cost['average_cost'] = ingredients[0].averageCost
          ...
          cost['average_effect'] = eff.avgeffectperparticipant
    ...
    return render(request,'reports/test.html', {'project_id':project_id,'ProgList':ProgList,'CostList':CostList})
...
def costeff(request):                                                                                                               
   return render(request,'reports/costeff.html')
...
def compcostanal(request):
   ...
   try:
      ...
      for p in program:
         ...
         ingredients = m.Ingredients.objects.filter(programId = p.id)
         try:
            ret['costperPart'] = ingredients[0].cost
            ret['total_cost'] = ingredients[0].totalCost
         except:
            ret['average_cost'] = 'n/a'
  99              ret['total_cost'] = 'n/a'
 100              cost['total_cost'] = 'n/a'                                                                                             
 101              ret['effRatio'] = 'n/a'
 102           ret['short_name'] = p.progshortname
 103           ret['lengthofprogram'] = programdesc.lengthofprogram
          ret['num_participants'] = programdesc.numberofparticipants
 105           ret['average_effect'] = eff.avgeffectperparticipant
 106           cost['average_effect'] = eff.avgeffectperparticipant
 107           ret['unitmeasureeffect'] = eff.unitmeasureeffect
 108           ret['sigeffect'] = eff.sigeffect
 109           
 110           ProgList.append(ret)
 111           CostList.append(cost)
 112     except ObjectDoesNotExist:
 113        return HttpResponse('A Project and/or Program does not exist! Cannot proceed further.')
 114     return render(request,'reports/test.html', {'project_id':project_id,'ProgList':ProgList,'CostList':CostList})                                                                                                               
   return render(request,'reports/compcostanal.html')   
...
def tabbedlayout(request,project_id,program_id):
    ...
    if request.method == 'POST':
        if 'submit1' in request.POST:
            if form1.is_valid():
               ...
               if partform.is_valid():
                  ...
                  total = 0
                  if (m.ParticipantsPerYear.objects.filter(programdescId=id.id).count() == 0):
       ...
       try: 
          exists = m.ParticipantsPerYear.objects.filter(programdescId=id.id)   
...
def project_list(request):
    ...
    if user != 'cbcse' or password != 'apr16':
